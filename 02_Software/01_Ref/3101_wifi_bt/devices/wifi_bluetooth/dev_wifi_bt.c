...
int WiFiBtDevConnect(ConnectInfo *info)
{
	...
	xprintf("Connection timed out or failed\n"); // 增加日志
    return -EIO;
}
...
{
	__disable_irq(); // 禁用中断
    pWiFiBtDev->Write(pWiFiBtDev, (unsigned char*)"AT+RST\r\n", strlen("AT+RST\r\n"));
    mdelay(5000); // 等待模块复位
	__enable_irq(); // 重新启用中断
}
